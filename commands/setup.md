...
4. Use pre-compiled `dist/index.js` for stability (works with both bun and node):
   ```
...
5. Generate command (quotes around runtime path handle spaces):
   ```
   bash -c 'plugin_dir=$(ls -d "$HOME"/.claude/plugins/cache/claude-hud/claude-hud/*/ 2>/dev/null | awk -F/ '"'"'{ print $(NF-1) "\t" $0 }'"'"' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n | tail -1 | cut -f2-); exec "{RUNTIME_PATH}" "${plugin_dir}dist/index.js"'
   ```
...
3. Use pre-compiled `dist\index.js` for stability (works with both bun and node).
...
**If yes**: Ask the user if they'd like to ⭐ star the claude-hud repository on GitHub to support the project. If they agree and `gh` CLI is available, run: `gh api -X PUT /user/starred/fxzer/claude-hud`. Only run the star command if they explicitly say yes.
